...
const paragraphe = document.getElementById('citation');
const auteur = document.getElementById('auteur')
const action = document.getElementById('nouveau')
...
const newQuote = () => {
    ...
    if (number != dernier) {
        newCitation = citations[number]
        console.log(newCitation);
        paragraphe.textContent = `${newCitation[0]}`
        auteur.textContent = `${newCitation[1]}`
        dernier = number
        console.log(dernier);
    }
    else number = Math.floor(Math.random() * Math.floor(max));

}
action.addEventListener('click', newQuote)
